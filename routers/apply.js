...
router.get("/cohorts/all-active", async (req, res) => {
  try {
    ...
    const activeCohorts = await Cohort.find({ isActive: true })
      .sort({ startDate: -1 }); // Show newest first
...
router.patch("/cohorts/:cohortId", protect, authorize("super-admin"), async (req, res) => {
  try {
    const { cohortId } = req.params;
    const updates = req.body;

    // Ensure deadline is not in the past
    if (updates.applicationDeadline && new Date(updates.applicationDeadline) <= new Date()) {
      return res.status(400).json({
        success: false,
        message: "Application deadline must be in the future",
      });
    ...
    const updatedCohort = await Cohort.findByIdAndUpdate(cohortId, updates, { new: true, runValidators: true });

    if (!updatedCohort) {
      return res.status(404).json({ success: false, message: "Cohort not found" });
    }

    res.status(200).json({ success: true, message: "Cohort updated", data: updatedCohort });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
...
router.delete("/cohorts/:cohortId", protect, authorize("super-admin"), async (req, res) => {
  try {
    const { cohortId } = req.params;
    const deleted = await Cohort.findByIdAndDelete(cohortId);

    if (!deleted) return res.status(404).json({ success: false, message: "Cohort not found" });

    res.status(200).json({ success: true, message: "Cohort deleted successfully", data: deleted });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Server error", error: error.message });
  }
...
router.get("/cohorts/:cohortId/applications", async (req, res) => {
  try {
    ...
    const applications = await ApplicationForm.find(query).populate("cohort", "name");
...
router.put("/:id/status", protect, authorize("super-admin", "admin"), async (req, res) => {
  try {
...
router.get("/stats/premium", async (req, res) => {
  try {
    const premiumApplications = await ApplicationForm.find({ package: "premium" });
...
router.post("/apply", async (req, res) => {
  try {
    const {
      ...
      paymentReference,
      package: pkg,
      cohortId, // Required now
    } = req.body;

    // Check if cohortId is provided
    if (!cohortId) {
      return res.status(400).json({
        success: false,
    ...
    // Check if email already applied to this cohort
    ...
    // Populate cohort details for email
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const pendingApplications = await ApplicationForm.countDocuments({ status: "Pending" }); // ✅ new
...
router.get("/cohorts/:cohortId/submissions", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    const filteredSubmissions = submissions.filter(sub => sub.application !== null);
...
router.delete("/submissions/:id", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    const application = await ApplicationForm.findById(submission.application);

    if (application) {
      // 2. Decrement completed tasks (ensure it doesn't go below 0)
      application.completedTasks = Math.max(0, application.completedTasks - 1);
      
      // 3. Recalculate progress
      const total = application.totalTasks || 10; // Defaulting to 10 if not set
      application.progress = Math.round((application.completedTasks / total) * 100);
