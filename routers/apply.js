...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // --- Resend SDK Logic ---
      // We wrap this in a try/catch so a failed email doesn't crash the status update
      try {
        if (status === "Approved") {
          ...
          console.log(`✅ Approval email sent to ${updatedApp.email}`);
        } else if (status === "Rejected") {
          await resend.emails.send({
            from: 'Knownly Internship <support@knownly.tech>',
            to: [updatedApp.email],
            subject: "Application Update - Knownly Internship",
            html: rejectedEmail(updatedApp.fname),
          });
          console.log(`✅ Rejection email sent to ${updatedApp.email}`);
        }
      } catch (mailError) {
        // We log the error but still return a 200 because the DB was updated
        console.error("❌ Admin Notification Mailer Error:", mailError.message);
      }
...
router.post("/apply", async (req, res) => {
  try {
    const {
      email, fname, lname, phone, social, university,
      track, level, package: pkg, cohortId,
    } = req.body;

    // 1. Validations
    if (!cohortId) return res.status(400).json({ success: false, message: "Cohort ID is required." });

    const cohort = await Cohort.findById(cohortId);
    if (!cohort) return res.status(404).json({ success: false, message: "Cohort not found." });

    if (!cohort.canAcceptApplications()) {
      return res.status(400).json({ success: false, message: `Applications for ${cohort.name} are closed.` });
    }

    const existingApplication = await ApplicationForm.findOne({ email, cohort: cohortId });
    if (existingApplication) {
      return res.status(400).json({ success: false, message: "You have already applied to this cohort." });
    }
        ...
        const { data, error } = await resend.emails.send({
          ...
          html: `
            <div style="font-family: 'Segoe UI', Roboto, Arial, sans-serif; background-color: #f4f7fa; padding: 40px 0; display: flex; justify-content: center;">
              <div style="background: #ffffff; max-width: 550px; width: 100%; border-radius: 10px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); padding: 30px 35px; margin: auto;">
                ...
                <p style="font-size: 15px; color: #374151; line-height: 1.6;">
                  ...
                  <strong>Knownly Internship Program (${cohortName})</strong> has been received successfully.
                </p>

                <div style="background: #f9fafb; padding: 15px 20px; border-radius: 8px; margin-top: 20px;">
                  <p style="font-size: 14px; color: #0f172a; margin: 0;">💎 <strong>Package:</strong> ${pkg}</p>
                  <p style="font-size: 14px; color: #374151; margin-top: 10px;">
                    ${pkg === 'Premium' 
                      ? "Please proceed to complete your <strong>subaccount transaction</strong> to secure your mentorship spot." 
                      : "You are on the Free Track. Stay tuned for onboarding details!"}
                  </p>
                  <div style="text-align: center; margin-top: 20px;">
                    <a href="https://knownly.tech/premium" style="background-color: #4f39f6; color: #ffffff; text-decoration: none; padding: 12px 22px; border-radius: 6px; font-weight: 500; display: inline-block;">Explore Premium Benefits</a>
                  </div>
                </div>

                <p style="font-size: 15px; color: #4b5563; line-height: 1.6; margin-top: 25px;">Best regards, <br/><strong>The Knownly Team</strong></p>
              </div>
            </div>
          `
        });
    ...
    const newApplication = await ApplicationForm.create({
      cohort: cohortId,
      email, fname, lname, package: "Free",
      phone, level, track, social, university,
      sendAt: new Date(Date.now() + 5 * 60 * 1000),
    });

    // CRITICAL: Await before sending response
    await triggerWelcomeEmail(cohort.name);
